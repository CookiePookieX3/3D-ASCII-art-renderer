...
int main(){
    ...
    Sphere* s0 = new Sphere(0,0 + 0.5*sin(6.28 * ms/1000 / 5),0,1);
    Sphere* s1 = new Sphere(3,0 + 0.2*sin(6.28 * ms/1000 / 0.5),0,0.5);
    ...
    while(dur < std::chrono::milliseconds(100000)){
        ...
        vec3 cameraAngle = vec3(0,0,0) - cameraPos;
        float matrixToCamera = 3;
        vec3 lightSource(10*sin(6.28 * ms/1000/10),0,10*cos(6.28 * ms/1000/10));

        s0->center.y = 0 + 0.5*sin(6.28 * ms/1000 / 5);
        s1->center.y = 0 + 0.2*sin(6.28 * ms/1000 / 0.5);
        s1->center.x = 0 + 3*sin(6.28 * ms/1000 / 3);
        s1->center.z = 0 + 3*cos(6.28 * ms/1000 / 3);
