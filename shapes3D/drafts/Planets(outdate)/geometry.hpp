...
struct Object{
    ...
    char brightness(float scalarMul){
        if(type == 0){
                if(scalarMul>0.98) return '@';
                if(scalarMul>0.95) return '$';
                if(scalarMul>0.9) return '8';
                if(scalarMul>0.85) return 'W';
                if(scalarMul>0.75) return 'H';
                if(scalarMul>0.7) return '9';
                if(scalarMul>0.65) return 'Z';
                if(scalarMul>0.6) return '4';
                if(scalarMul>0.55) return '1';
                if(scalarMul>0.5) return 'l';
                if(scalarMul>0.4) return 'r';
                if(scalarMul>0.2) return '(';
                if(scalarMul>0.15) return '/';
                if(scalarMul>0.1) return '!';
                if(scalarMul>-0.3) return '.';
                return ' ';
        } else if(type == 1){
                if(scalarMul>0.98) return '@';
                if(scalarMul>0.95) return '$';
